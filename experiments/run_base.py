...
exp_configs = [
    ...
    ('experiment_DynaQNN_Aug11_base.py', 1, 1),
...
def build_cmd(exp_file, num_sim, num_log, filecnt):
    log_file = '_'.join(['msg'] + exp_file.split('_')[1:3] + [FLAG,str(filecnt)])
    type_agent = exp_file.split('_')[1]
    exp_list = ['python ./' + exp_file + ' ' + str(i) + ' ' + FLAG + ' ' + str(filecnt) for i in range(num_log)]
...
runs = [build_cmd(*exp_configs,filecnt) for filecnt in range(99)]
